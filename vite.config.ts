                    ...
                    try {
                        ...
                        if (parsed.mode === 'vision-product') {
                            const { status, body: out } = await runVisionProductSearch({
                                ...
                                // 参考画像クリック経路（本番 api/agent.ts と同一・260728）。
                                // 画像もページも、受け取るのは URL ではなく署名トークン（任意URLの取得代行にしないため）。
                                imageToken:
                                    typeof (parsed as { imageToken?: unknown }).imageToken === 'string' ? (parsed as { imageToken?: string }).imageToken : '',
                                pageToken:
