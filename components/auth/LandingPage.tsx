 ...
 * 実績数値 → AIデザイン編集 → AIエージェント（各リードコピー＋デモ枠・260619 クライアントデザイン）→ 比較表（Arise vs 汎用AI vs 3D/CAD・260625）→ よくある質問（FAQ・260626）→ お客様からの声（導入事例・260626）→ クロージング（招待制）→ フッター。
...
// よくある質問（FAQ・260626 クライアント支給原稿）。a は Jp 用に文節で分割（原稿の文言は不変、分割のみ）。
const FAQS = [
  {
    q: '生成される画像は、実際の図面や寸法とズレませんか？',
    a: [
      '既存の汎用画像生成AIとは異なり、',
      'ARISEは図面（2D/3Dエディタ）の構造をベースに生成を行います。',
      'そのため、空間のプロポーションや壁・窓の配置といった',
      '物理的な制約を正確に保持したまま、',
      'リアルなイメージを出力することが可能です。',
    ],
  },
  {
    q: '3DソフトやAIの専門知識（プロンプト等）は必要ですか？',
    a: [
      '一切不要です。',
      'ブラウザ上で直感的に操作できるノーコード設計のため、',
      '複雑なプロンプト（呪文）の入力技術がなくても、',
      'クリックと簡単なテキスト入力だけで、',
      '初日からプロ品質のイメージを作成・編集いただけます。',
    ],
  },
  {
    q: '生成した画像は商用利用可能ですか？著作権はどうなりますか？',
    a: [
      'はい、商用利用可能です。',
      '生成された画像の権利はユーザー様（ご契約者様）に帰属します。',
      '顧客へのプレゼン資料、自社サイトのポートフォリオ、',
      '広告チラシ等にそのままご活用いただけます。',
    ],
  },
  {
    q: '無料プランでは、どこまで機能を使えますか？',
    a: [
      '基本的な2D/3D編集機能、AI編集機能に加え、',
      '無料で付与されるトークン範囲内でAIパース生成をお試しいただけます。',
      '実務での本格的な導入前にクオリティを存分にご確認ください。',
    ],
  },
  {
    q: 'パースの生成から「概算見積もりの自動算出」までは、どのように連動するのですか？',
    a: [
      'エディタ上であらかじめ配置した建材や家具のデータ',
      '（面積や個数、実在するメーカーの品番など）を',
      'システムが自動で読み取り、リアルタイムで概算費用を算出します。',
      'また、AI編集機能を用いてイメージに反映した家具などについては',
      '別途手入力で追記をしていただくことも可能です。',
      'デザインの変更が見積もりに即座に反映されるため、',
      '商談のタイムラグが消滅します。',
    ],
  },
] as const;

// お客様からの声（導入事例・260626 クライアント支給原稿）。body は Jp 用に文節で分割（文言は不変）。
const TESTIMONIALS = [
  {
    label: '導入事例 1：デザインと時短のジレンマを解消',
    quote: '施主の『ここを変えたい』に、その場で応えられるようになりました',
    who: '建築事務所 設計担当（30代）',
    body: [
      '今までパース作成は外注に頼るか、',
      '重い3Dソフトと格闘して何時間もかけていました。',
      'ARISEを導入してからは、施主との打ち合わせ中にその場で',
      '「壁紙を木目に」「照明を暖色に」とピンポイントでAI編集して見せられるようになり、',
      '即決いただける案件が劇的に増えました。',
      'AI特有の「寸法の嘘」がないのが本当に助かります。',
    ],
  },
  {
    label: '導入事例 2：商談スピードの劇的な向上',
    quote: 'パース作成から見積もりまでのタイムラグが『ゼロ』に',
    who: '工務店 営業ディレクター（40代）',
    body: [
      'デザインの美しさもさることながら、',
      'パースと連動して「概算見積もり」まで自動算出される機能は革命的です。',
      'これまでは「持ち帰って後日お見積もりを出します」というフローで',
      '商談の熱が冷めてしまうことがありましたが、',
      '今はお客様と画面を見ながらその場で予算のすり合わせが完了します。',
      'もう手放せません。',
    ],
  },
  {
    label: '導入事例 3：教育コスト削減と汎用AIからの乗り換え',
    quote: 'プロが実務で使うために設計された、唯一の選択肢',
    who: '内装デザイン会社 代表（50代）',
    body: [
      '複数の汎用AIも試しましたが、',
      '空間の構造を無視した絵になるため実務では使い物にならず挫折していました。',
      'Ariseは直感的な操作感ながら、',
      '建築の正確性をしっかり担保してくれます。',
      '若手スタッフでも初日からプロ顔負けのビジュアルを作れるため、',
      '社内の教育コストも大幅に削減できました。',
    ],
  },
] as const;
