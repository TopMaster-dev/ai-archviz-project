  ...
  */
        ...
        <div className="absolute top-6 left-6 right-6 z-50 flex flex-wrap items-start justify-between gap-2 pointer-events-none">
          ...
          {!photoOnly && (
            <button
              type="button"
              disabled
              className="pointer-events-auto xl:absolute xl:top-0 xl:left-1/2 xl:-translate-x-1/2 shrink-0 flex items-center justify-center gap-2 px-4 sm:px-8 py-3 rounded-2xl bg-emerald-950/80 border border-emerald-500/40 text-emerald-100 shadow-[0_8px_24px_rgba(16,185,129,0.25)] transition-all disabled:opacity-35 disabled:cursor-not-allowed"
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex flex-col bg-zinc-950 text-white pl-3 pr-0 pt-0 pb-0 gap-3">
      ...
      {/* 上部バー: モード切替（左）＋ 書き出し。1280px以上(xl)は元の中央固定（右レールの概算見積DLボタンと重ならないため）、
          1280px未満は右寄せ(justify-between)。狭幅は折り返す（260727・xlで元配置へ戻す）。 */}
      <div className="absolute top-6 left-6 right-6 z-50 flex flex-wrap items-start justify-between gap-2 pointer-events-none">
        {renderModeBarOrHome()}
        <button
          type="button"
          ...
          className="pointer-events-auto xl:absolute xl:top-0 xl:left-1/2 xl:-translate-x-1/2 shrink-0 flex items-center justify-center gap-2 px-4 sm:px-8 py-3 rounded-2xl bg-emerald-950/80 border border-emerald-500/40 text-emerald-100 shadow-[0_8px_24px_rgba(16,185,129,0.25)] hover:bg-emerald-900/80 transition-all disabled:opacity-35 disabled:cursor-not-allowed"
        >
          <Download className="w-4 h-4 shrink-0" />
          <span className="text-[11px] font-black uppercase tracking-widest">この画像を書き出し</span>
        </button>
