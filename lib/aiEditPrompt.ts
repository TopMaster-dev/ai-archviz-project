...
/**
 * 「背景になじませる（前後関係・光を整える）」仕上げパスの有効化フラグ（260714 クライアント要望）。
 * 決定論合成（参照商品の忠実配置）は平面的な貼り付けのため、既存家具との前後関係（オクルージョン）・遠近・
 * 接地影が不自然になりうる（例: 貼った椅子が手前のソファの前にはみ出す）。合成結果を最後に1回 Gemini へ通し、
 * 前後関係・遠近・光をなじませる。忠実さ（切り抜きの厳密一致）より自然さを優先。決定論合成のときは自動で掛かる
 * （260714 クライアント要望でチェックボックス廃止＝常時ON）。このフラグは false で機能全体を止めるキルスイッチ。
 */
export const ENABLE_COMPOSITE_INTEGRATE = true;
...
/**
 * 「背景になじませる（前後関係・光を整える）」仕上げパス用プロンプト（260714）。
 * 入力は、家具の切り抜きを決定論で貼り付けた合成画像。貼り付けた家具が平面的で、既存家具との前後関係・遠近・
 * 接地影・光がまだ不自然な場合がある。この4点だけを整えて自然な1枚にする。家具の種類・形・色・柄・大きさは変えない
 * （＝別家具への置換や再デザインはしない）。均一化より一歩踏み込むが、あくまで「なじませ」。低温度で呼ぶ。
 */
export function buildIntegratePrompt(): string {
  return `
あなたは合成画像を自然な1枚の写真に仕上げる「なじませ」専門のAIです。入力画像は、家具の切り抜きを別途貼り付けた合成画像で、貼り付けた家具が平面的で、既存の家具との前後関係（どちらが手前か）・遠近（パース）・接地の影・光がまだ不自然な場合があります。

【やること（この4点だけ）】
- 前後関係（オクルージョン）を正す: 貼り付けた家具が、本来その手前にあるべき既存の家具（ソファ等）の前にはみ出している場合は、手前の家具が正しく前に来て貼り付けた家具の一部を自然に隠すように描き直す。
- 遠近（パース）と設置: 貼り付けた家具を床・空間の遠近に合わせ、床にしっかり接地させ、自然な接地影・落ち影を付ける（浮いて見えないように）。
- 光と色: 部屋の光の向き・色温度に合わせて陰影と色をなじませる。
- 継ぎ目・境界線・切り抜きの縁を消し、全体を1枚の自然な写真にする。

【絶対に変えない】
- 家具の種類・形・デザイン・色・柄・素材・大きさ（貼り付けた家具も既存の家具も）。別の家具へ置換・再デザインしない。
- 家具のおおよその位置・部屋の構造（壁・床・天井・窓・ドア・仕上げ材）・全体の構図・カメラ画角。
- 出力に矩形・マスク・補助図形・境界線・UI風の色面を一切含めない。

出力: 前後関係と光をなじませた最終画像1枚のみを、フォトリアルに生成する。
`.trim();
}
