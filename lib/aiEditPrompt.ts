...
- 壁・床・天井の仕上げを別素材に差し替えたり、カメラや空間の幾何を変えない。`
  ...
  const objectRefLine = hasObjectRefs
    ? '各オブジェクト参照画像を、テキストで指定した領域（正規化座標）に従って合成する。領域ごとに、シルエット・アームの形状・脚・生地のリブや柄・プロポーションなど、家具・小物の見た目のデザインは参照画像を正とする。ただし設置の向き・角度・カメラに対する見え方は参照画像ではなく、ベース画像の該当領域（元の家具）とパースに合わせる（参照画像が正面写真でも、元が斜め向きなら斜め向きで描く）。**差し替え後の家具の向き・角度・姿勢（正面／斜め／横向き・カメラに対する見え方・脚や座面の接地の向き）は、差し替え前にその場所に写っていた家具と必ず一致させ、勝手に正面化・回転・反転・左右反転をしない。**ベース該当領域の既存家具の輪郭を保持したままの上塗りやテクスチャ転写、および形状の妥協的合成は行わない。**【差し替え対象は“枠”で切らない・ただし元の位置と遮蔽は必ず保つ（最優先）】参照画像で差し替える家具は、囲った領域（フォーカス枠）を「注目する対象」の目印として、その対象が今写っている位置・向き・大きさのまま、参照画像のデザインへ差し替える。フォーカス枠は編集の目印にすぎず、対象が枠より大きければ枠の外へ自然にはみ出してよい（＝枠の縁で切り取って部分表示にする“見切れ”は禁止）。ただし「全体を見せる」ことを理由に、対象を元の位置から動かしたり、手前の家具（ソファ・テーブル等）の前へ引き出したり、空いた床・空間へ“もう1脚”を追加・複製したりしては絶対にならない。元々ソファ等の手前の家具に一部隠れている対象は、その遮蔽（隠れて見えない範囲）をそのまま保ち、見えている部分だけをその場で差し替える（隠れた部分を勝手に描き足して全身を露出させない・手前へ移動して露出させない）。ここで言う「見切れ禁止」は“描画フレーム（囲み枠）で切らない”という意味であり、手前の実在家具による自然な前後の隠れ（遮蔽）はそのまま維持する。【狭い場所・コーナーは範囲の大きさに合わせて縮小する】差し替え対象が、壁際・部屋のコーナー・家具どうしの狭い隙間など、その場所に自然な大きさでは物理的に収まらない場合は、無理に大きいまま描いて壁や隣の家具へめり込ませたり画面外へはみ出させたりせず、囲った範囲（＝そこに置ける空間の目安）に収まるよう対象のサイズを縮小し、その場に自然に収める（位置は動かさず・向きは保ち・大きさだけ調整する）。逆に十分な空間がある通常の配置では、無理に枠へ押し込めず自然な大きさで置く。縮小するのは「対象を置く“床・空間そのもの”が物理的に狭い（コーナー・壁際・家具どうしの隙間）」ときだけである。囲み枠が小さいのが単に「手前の家具に一部隠れて可視部分が小さいだけ（遮蔽）」のときは縮小しない＝対象は本来の大きさのまま、隠れる部分は隠したまま差し替える（遮蔽と“狭い空間”を混同しない）。**領域外はベース画像を完全に保持し、指示にない既存の要素（家具・植物・装飾・小物・ドライフラワー等）を新たに追加・複製・増殖・拡大・改変しない。**配置座標と別途記載の短い日本語説明が食い違う場合は、位置・スケールの解釈では座標を必ず優先する。形状・材質の参照はオブジェクト参照画像に従う。**'
