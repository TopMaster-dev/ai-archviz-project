...
- 壁・床・天井の仕上げを別素材に差し替えたり、カメラや空間の幾何を変えない。`
  ...
  const objectRefLine = hasObjectRefs
    ? '各オブジェクト参照画像を、テキストで指定した領域（正規化座標）に従って合成する。領域ごとに、シルエット・アームの形状・脚・生地のリブや柄・プロポーションなど、家具・小物の見た目のデザインは参照画像を正とする。ただし設置の向き・角度・カメラに対する見え方は参照画像ではなく、ベース画像の該当領域（元の家具）とパースに合わせる（参照画像が正面写真でも、元が斜め向きなら斜め向きで描く）。ベース該当領域の既存家具の輪郭を保持したままの上塗りやテクスチャ転写、および形状の妥協的合成は行わない。**【差し替え対象は必ず全体を表示・見切れ厳禁（最優先）】参照画像で差し替える家具・オブジェクトは、背もたれ・肘掛け・脚・脚先・上端・下端まで“全体”を欠けなく描く。囲った領域（フォーカス枠）は位置の目印にすぎず、対象が枠より大きい場合は枠の外へはみ出して構わないが、対象そのものを枠の縁で切り取って部分表示（見切れ）にしては絶対にならない。対象が枠に収まりきらないときは、フォトリアルに自然なスケールを保ったまま、画面内に対象の全体が収まるよう配置・サイズを調整し、決して途中で断ち切らない。**領域外はベース画像を完全に保持し、指示にない既存の要素（家具・植物・装飾・小物・ドライフラワー等）を新たに追加・複製・増殖・拡大・改変しない。**配置座標と別途記載の短い日本語説明が食い違う場合は、位置・スケールの解釈では座標を必ず優先する。形状・材質の参照はオブジェクト参照画像に従う。**'
