...
describe('画質を高める（精細化・260710）: 単一画像の後処理・内容不変・見本画像を使わない', () => {
  ...
  /**
   * 260731 クライアント指摘「高解像度書き出しで白かったカウンターが木目に変わった」。
   *
   * 旧文面は「素材の質感（木目・織り・石・金属など）」と木目を最初の例に挙げ、さらに
   * 「のっぺりした面に…凹凸・陰影を戻す」と指示していた。白い天板はまさに『のっぺりした面』で、
   * かつ“元の柄”が読み取れないため、モデルは学習事前分布（天板＝木目）で埋めてしまう。
   * 木目化は偶然ではなく、この2つの指示から構造的に起きていた。
   */
  it('素材名を例示しない（木目をモデルへ刷り込まない）', () => {
    expect(p).not.toContain('木目・織り・石・金属');
  });

  it('のっぺりした面を埋めさせない（白い天板が木目になる直接原因だった）', () => {
    expect(p).not.toContain('のっぺりした面に');
  });

  it('無地の面は無地のまま保つよう明示する', () => {
    expect(p).toContain('無地');
    expect(p).toContain('模様を描き足さない');
  });

  it('素材を守る対象が壁・床・天井に限定されていない（天板・家具も守る）', () => {
    // 旧文面は主語が『壁・床・天井の仕上げ材』だけで、カウンター天板がどの保護にも掛からなかった。
    expect(p).toContain('カウンター天板');
    expect(p).toContain('面すべて');
  });

  it('存在しないディテールを創作させない', () => {
