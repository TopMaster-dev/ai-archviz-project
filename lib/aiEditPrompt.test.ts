...
describe('isSurfacePlaneFinish＝面仕上げはクロップせず全画面生成（Path A・③④・260717）', () => {
  ...
  it('面仕上げプロンプトに「塗り残さない・枠のきわまで／窓建具は残す」が入る（③・開口は覆わない）', () => {
    ...
    // 開口（窓・ドア・ガラス・建具）そのものは覆わず残す（誤って塗りつぶさない）。
    expect(guide).toContain('元の見た目のまま残す');
    // 「システムが自動で復元する」という誤った前提の文言は無い（面外クリップは開口を復元しない）。
    expect(guide).not.toContain('自動で元へ戻す');
  });
