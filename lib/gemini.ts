...
各 objectId について、次を1行（120文字以内）に凝縮します: (1)フォーカス領域が指す“差し替える対象”（例: 奥の白いラウンジチェア）(2)画面上のおおまかな位置と距離感（例: 中央やや右・中景）(3)向き（例: 正面がやや左向き）(4)前後関係と、触らず保持すべき手前の家具（重なりがあれば、対象が手前か奥か・手前に何が重なっているか。例: 手前にソファが重なる＝ソファは保持）。
重要: 対象は必ずしも最前面とは限らない。手前の家具の後ろに一部隠れている“奥の対象”でも、その対象自体を差し替える前提で分析し、対象を空いた場所へ動かす想定はしない。位置・向きは原則そのまま維持する前提。座標の数値は繰り返さない。判断が難しい項目は書かない（誤った断定はしない）。
