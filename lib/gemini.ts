...
【誤検出を避ける（重要）】次のものは開口ではないので openings に含めない: 鏡・姿見、絵画・写真・ポスター・タペストリー・壁掛けアート、テレビ・モニター・スクリーン、時計、棚・キャビネット・収納の扉、壁の装飾パネルや飾り、単に色や明るさが違うだけの壁面、影・映り込み。さらに【天井まわりは特に厳しく除外する】: LED間接照明・コーブ照明の光の帯、折上げ天井や下がり天井（ソフィット）の見切り・段差、廻り縁・モールディング・幕板、天井と壁の境目、天井際の明るい帯や光だまり——これらは窓・ドアではないので絶対に含めない。実在の窓・ドアは壁面の中ほど（またはそれより下）まで縦に伸び、下に窓台・敷居や床がある。天井にぴったり接した薄い横長の帯・光の筋・段差は窓ではないので挙げない。判断に迷う矩形は列挙しない（“本物の窓・ドアだと確信できるものだけ”を挙げ、疑わしきは含めない＝取りこぼしの方が誤検出より安全）。開口が無ければ openings は空配列 []。フォーカス領域が面（壁/床/天井）でない（家具の差し替え等）場合も空配列。
