...
/**
 * 公開画像を 1 本だけ安全に取得し、base64 で返す。
 *
 * 【現在の呼び出し元】無し（260729）。これを使っていた「参考画像クリックで再検索」は
 * クライアント要望で廃止した。残してあるのは、候補画像をサーバで取得して
 * 元画像と見比べる（見た目の再ランク付け）案が保留中で、その際に必要になるため。
 * 使うときはページ取得と同じゲートを通っていることを前提にしてよい。
 *
 *
 * なぜサーバ側で取りに行くのか:
