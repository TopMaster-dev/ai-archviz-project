...
// 本番サーバーレス関数。中核ロジックは lib/aiAnalyzeCore.ts（開発 vite.config.ts と共有＝挙動差ゼロ・260709）。
// エリア編集の事前解析（対象の説明＋遮蔽判定 occluded）を、生成本体より前に単独で取得するためのエンドポイント。
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const result = await runAiAnalyze(apiKey, req.body ?? {});
    if (!result.success) {
      return res.status(result.status).json({ success: false, error: result.error });
    }
    return res.status(200).json({ success: true, narratives: result.narratives, occluded: result.occluded });
  } catch (e: any) {
