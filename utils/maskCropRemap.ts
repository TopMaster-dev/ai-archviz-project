...
// 注（260709）: エリア編集の出し分けは「対象が別家具の後ろに隠れている（遮蔽・occluded）ときだけクロップ（案1）、
// 通常は自然な全画面（案2）」に一本化した（AiEditWorkspace.runEdit・遮蔽判定は /api/ai-edit の analyze:true）。これに伴い旧フラグ
// ENABLE_FULLFRAME_ONLY / PRESERVE_OUTSIDE_MASK / SHOW_CONFINE_TOGGLE（範囲外固定トグル関連）は撤去した。
// isLargeRegion は「大領域はクロップ利得が無いのでクロップしない」判定に引き続き使う。

/** union マスク外接矩形が「大領域」か（＝クロップしても利得が無いので全画面編集を使う）。純関数。 */
