 ...
 */
// 260704: テストマーケ期間中は一旦オフにしていたが、260715 クライアント要望で本格運用として ON に戻す。
// 260718: クライアント要望により「一旦」書き出し画像の透かしを OFF に（＝高解像度DLの回数制限・透かしを無効化）。
//   本フラグ false で isActive が常に false になり、書き出しに透かしが入らず・回数UIも非表示・カウントも no-op。
//   本格運用として透かし/回数制限を再開する際は true に戻すだけでよい（挙動・カウント仕様はそのまま）。
// 無料プラン（ログイン済み）の高解像度DLは、月3回まで無透かし・4回目以降に「フリープラン サンプル」透かしを合成する（ON時）。
// ※ 生成画像全体への透かし/縮小（ENABLE_FREE_PLAN_OUTPUT_LIMITS）は別機能で、こちらは据置（false）。
// ※ ゲスト（未ログイン）・有料プラン・プレビュー等倍保存は対象外（無透かし・無制限）。
