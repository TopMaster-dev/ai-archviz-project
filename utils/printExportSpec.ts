...
/**
 * 手段1（260814 クライアント承認）: 生成結果を版の寸法へリサイズせず、
 * モデルが返した寸法のまま採用する（縦横比がずれた場合だけ補正）。
 *
 * 1回の編集ごとに入っていたリサンプル（損失A）が消える。
 * 版が生成より小さければ縮小、大きければ拡大されていた。特に拡大は
 * 補間で存在しない画素を作ってからぼかすため実害が大きかった。
 *
 * 適用先は**合成（マスク座標）を伴わない経路のみ**（エリア編集の全画面採用・コーディネート）。
 * 合成経路は baseW/baseH が座標の基準なので触っていない。
 *
 * false に戻すと従来どおり版の寸法へ合わせる挙動へ即座に戻る（1行ロールバック）。
 * 実機で「構図が変わっていないこと」「旧プロジェクトで解像度が上がること」を
 * 確認してから true を既定にすること。
 */
export const ENABLE_KEEP_GENERATED_SIZE = true;

/**
 * エリア編集/コーディネートの「土台画像」を縮小する長辺上限。
 *
 * 【手段2・260814 クライアント承認】生成長辺と一致させる。
 * これらの経路は生成結果を土台(base)の寸法へ合わせるため、
 * 土台が生成結果より大きいと、そのぶん必ず**拡大**リサイズが入る。
 * 拡大は補間で存在しない画素を作る処理なので、編集のたびに眠さが積み上がる（損失A）。
 * 上限を生成長辺そのものに固定すれば、土台が生成を上回ることが無くなり、
 * リサイズが原理的に発生しなくなる。
 *
 * 旧値は 3072（260722 に 2048 から引き上げたもの）。2688 の生成に対して常に上振れの余地があった。
 *
 * 副次効果: 送信は compressDataUrlToBudget が 2.2MB 固定予算でバイト圧縮するため、
 * 画素数が減ったぶん同じ容量で JPEG 品質を上げられる（圧縮由来の損失Bが軽くなる）。
 * 名目上の解像度は下がるが、減るのは補間で作った画素なので実質的な情報量は落ちない。
 *
 * ENABLE_2K_PREVIEW を 1K へ戻した場合も自動で追従する（片方だけ取り残されない）。
 */
export const AREA_EDIT_BASE_MAX_SIDE = PREVIEW_GEMINI_LONG_EDGE;
