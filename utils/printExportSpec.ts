...
/**
 * 【260820】Gemini が 2K で実際に返す寸法（比率ごと）。
 *
 * 「AIで写真編集」の版1は利用者の写真なので、寸法をこちらで決める必要がある。
 * 図面PJは版1が生成画像そのものなので定義上ずれないが、写真PJは合わせないとずれる。
 * 版1と生成結果の幾何が違うと次の2つが同時に起きる:
 *   1. モデルへの入力と要求出力の解像度が食い違い、モデルが描き直しやすくなる
 *      （手段2 で図面PJに起きたのと同じ条件）
 *   2. 生成結果を版寸法へ cover で合わせる際に毎回中央クロップが入り、編集ごとに蓄積する
 *      （16:9 の写真で 0.8%/回・10回で画角92.6%）
 *
 * 【厳守】ここは必ず実測値のみを入れること。理論値・推測を入れてはならない。
 * 生成サイズを推測したことが 手段2（2688）と normalizeAiEditBase（2688）の両方の原因だった。
 * 未実測の比率は載せない。載っていない比率は長辺のみを合わせる暫定動作になる。
 *
 * 測定方法: 各比率で1枚生成し、保存された画像の naturalWidth/Height を記録する。
 * 画像モデル（GEMINI_IMAGE_MODEL）を変えたら測り直すこと。
 */
export const MEASURED_GEMINI_2K_SIZES: Record<string, { w: number; h: number }> = {
  // クライアント実ファイル2件（概算見積_20260810.xlsx / _20260812.xlsx の埋め込み画像）で確認。
  '16:9': { w: 2752, h: 1536 },
  // 以降は未実測。測定でき次第ここへ追加すると、その比率も厳密一致になる。
};
